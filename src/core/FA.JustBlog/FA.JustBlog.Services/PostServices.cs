...
namespace FA.JustBlog.Services
{
    public class PostServices : BaseServices<Post>, IPostServices
    {
        ...
        {
            return await _unitOfWork.PostRepository.GetQuery().CountAsync(p => p.PostTags.Any(pt => pt.Tag.Name == tag));
        }
        ...
        {
            return await _unitOfWork.PostRepository.GetQuery().Where(p => p.PostTags.Any(t => t.Tag.Name == tag)).ToListAsync();
        }

        public async Task<IEnumerable<Post>> GetPostsByTagAsync(Guid tagId)
        {
            return await _unitOfWork.PostRepository.GetQuery().Where(p => p.PostTags.Any(t => t.TagId == tagId)).ToListAsync();
        }
